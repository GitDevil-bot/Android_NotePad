...
public class NotesList extends ListActivity {
    ...
    private String mCurrentCategory = "All"; // Current category filter
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Creates the backing adapter for the ListView with custom view binder
        mAdapter = new SimpleCursorAdapter(
                      this,                             // The Context for the ListView
                      R.layout.noteslist_item,          // Points to the XML for a list item
                      mCursor,                           // The cursor to get items from
                      dataColumns,
                      viewIDs
              ) {
            @Override
            public void setViewText(TextView v, String text) {
                if (v.getId() == R.id.text2) {
                    ...
                    try {
                        ...
                        text = sdf.format(new Date(timestamp));
            ...
            public void bindView(View view, Context context, Cursor cursor) {
                ...
                // Set todo label and highlight
                TextView todoLabel = (TextView) view.findViewById(R.id.todo_label);
                TextView titleView = (TextView) view.findViewById(android.R.id.text1);
                if (isTodo == 1) {
                    todoLabel.setVisibility(View.VISIBLE);
                    titleView.setTextColor(Color.parseColor("#FF5252")); // Red color for todo
                } else {
                    todoLabel.setVisibility(View.GONE);
                    // Set text color based on background
                    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
                    String bgColor = prefs.getString("background_color", "white");
                    int textColor = getTextColorForBackground(bgColor);
    ...
    protected void onResume() {
        super.onResume();
        // Reapply background color and refresh list
        applyBackgroundColor();
        // Refresh cursor to show updated data
    ...
    /**
     * Shows dialog to select background color
     */
    private void showBackgroundColorDialog() {
        ...
        String currentColor = prefs.getString("background_color", "white");
        int selectedIndex = 0;
        for (int i = 0; i < colors.length; i++) {
            if (colors[i].equalsIgnoreCase(currentColor) || 
                (currentColor.equals("white") && i == 0) ||
                (currentColor.equals("yellow") && i == 1) ||
                (currentColor.equals("blue") && i == 2) ||
                (currentColor.equals("black") && i == 3)) {
                selectedIndex = i;
                break;
            }
        }
        ...
        builder.setTitle(R.string.background_color_dialog);
        builder.setSingleChoiceItems(colors, selectedIndex, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                String colorName = colors[which].toLowerCase();
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(NotesList.this);
    ...
    /**
     * Shows dialog to filter by category
     */
    private void showCategoryFilterDialog() {
        final String[] categories = {
            ...
            getString(R.string.category_ideas),
            getString(R.string.category_todo)
        };
        ...
        if (mCurrentCategory.equals("All")) {
            ...
            for (int i = 1; i < categories.length; i++) {
                if (categories[i].equals(mCurrentCategory)) {
                    selectedIndex = i;
                    break;
                }
            }
        ...
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(this);
        builder.setTitle(R.string.menu_category_filter);
        builder.setSingleChoiceItems(categories, selectedIndex, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                // Refresh the list with new category filter
                String query = mSearchView != null ? mSearchView.getQuery().toString() : "";
                performSearch(query);
                dialog.dismiss();
            }
    ...
    /**
     * Applies background color from preferences
     */
    private void applyBackgroundColor() {
        ...
        switch (bgColor) {
            case "yellow":
                colorResId = R.color.background_yellow;
                break;
            case "blue":
                colorResId = R.color.background_blue;
                break;
            case "black":
                colorResId = R.color.background_black;
                break;
            default:
                colorResId = R.color.background_white;
                break;
        }
        
        getListView().setBackgroundColor(getResources().getColor(colorResId));
    }
    
    /**
     * Gets text color based on background color
     */
    private int getTextColorForBackground(String bgColor) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.menu_add) {
            ...
            return true;
        } else if (item.getItemId() == R.id.menu_background_color) {
            showBackgroundColorDialog();
            return true;
        } else if (item.getItemId() == R.id.menu_category_filter) {
