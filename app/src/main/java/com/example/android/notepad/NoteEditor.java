...
public class NoteEditor extends Activity {
    ...
    public static class LinedEditText extends EditText {
        ...
        public LinedEditText(Context context, AttributeSet attrs) {
            ...
            mPaint.setColor(0xFFE0E0E0); // Light gray lines for better readability
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // Update todo menu item state
        if (mCursor != null && mCursor.moveToFirst()) {
            int colTodoIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_IS_TODO);
            int isTodo = colTodoIndex >= 0 ? mCursor.getInt(colTodoIndex) : 0;
            MenuItem todoItem = menu.findItem(R.id.menu_todo);
            if (todoItem != null) {
                todoItem.setChecked(isTodo == 1);
            }
        }
    ...
    /**
     * Toggles todo status of the note
     */
    private void toggleTodo() {
        ...
        int colTodoIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_IS_TODO);
        int currentTodo = colTodoIndex >= 0 ? mCursor.getInt(colTodoIndex) : 0;
        int newTodo = (currentTodo == 1) ? 0 : 1;
        
        ContentValues values = new ContentValues();
        values.put(NotePad.Notes.COLUMN_NAME_IS_TODO, newTodo);
        getContentResolver().update(mUri, values, null, null);
        
        Toast.makeText(this, 
            newTodo == 1 ? R.string.todo_marked : R.string.todo_unmarked, 
    ...
    /**
     * Shows a dialog to select category for the note
     */
    private void showCategoryDialog() {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            int colIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_CATEGORY);
            if (colIndex >= 0) {
                currentCategory = mCursor.getString(colIndex);
    ...
    /**
     * Exports the current note to a text file
     */
    private void exportNote() {
        ...
        try {
            int colTitleIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_TITLE);
            int colNoteIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_NOTE);
            int colModDateIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE);
            
            String title = mCursor.getString(colTitleIndex);
            String note = mCursor.getString(colNoteIndex);
            long modDate = mCursor.getLong(colModDateIndex);
            ...
            // Create file name from title and timestamp
            String fileName = title.replaceAll("[^a-zA-Z0-9]", "_") + "_" + 
                            DateFormat.format("yyyyMMdd_HHmmss", modDate).toString() + ".txt";
            File exportFile = new File(exportDir, fileName);
            
            // Write note to file
            FileWriter writer = new FileWriter(exportFile);
            writer.write("Title: " + title + "\n\n");
            writer.write("Modified: " + DateFormat.format("yyyy-MM-dd HH:mm:ss", modDate).toString() + "\n\n");
            writer.write("Content:\n" + note);
            writer.close();
            
            Toast.makeText(this, 
                R.string.export_success + "\n" + getString(R.string.export_location) + ": " + 
